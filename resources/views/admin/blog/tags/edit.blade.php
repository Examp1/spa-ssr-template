...
@section('content')
    <div class="top_textWrp">
        ...
        <nav aria-label="breadcrumb" class="breadcrumb-nav">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item"><a href="{{ route('blog.tags.index') }}">{{ __('Tags') }}</a></li>
                <li class="breadcrumb-item active" aria-current="page">{{ __('Edit') }}</li>
            </ol>
        </nav>
    </div>

    <form class="form-horizontal" method="POST" action="{{ route('blog.tags.update', $model->id) }}">
        @method('PUT')
        @csrf
        <div class="row">
            <div class="col-md-9">
                        ...
                        <ul class="nav nav-tabs" id="myTab" role="tablist"
                            ...
                            <li class="nav-item">
                                <a class="nav-link active" id="main-tab" data-toggle="tab" href="#main" role="tab"
                                    aria-controls="main" aria-selected="true">Главное</a>
                            </li>
                            <li class="nav-item">
                                ...
                                <div class="tab-content">
                                    @foreach ($localizations as $key => $catLang)
                                        <div class="tab-pane p-t-20 p-b-20  @if (config('translatable.locale') == $key) active @endif"
                                            ...
                                            @include('admin.blog.tags.tabs._main', [
                                                'lang' => $key,
                                                'data' => $data,
                                                'model' => $model,
                                            ])
                                    ...
                                    <div class="tab-pane p-t-20 p-b-20" id="copy_lang" role="tabpanel">
                                        @include('admin.pieces._copy-lang', [
                                            'model_id' => $model->id,
                                            'route' => route('blog.tags.copy-lang'),
                                        ])
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="tab-content tabcontent-border">
                                    @foreach ($localizations as $key => $catLang)
                                        <div class="tab-pane p-t-20 p-b-20  @if (config('translatable.locale') == $key) active @endif"
                                            ...
                                            @include('admin.blog.tags.tabs._seo', [
                                                'lang' => $key,
                                                'data' => $data,
                                            ])
                                        </div>
    ...
    @can('blog_tags_delete')
        <form action="{{ route('blog.tags.destroy', $model->id) }}" method="POST" class="delete-model-form">
            @csrf
            @method('DELETE')
        </form>
    @endcan
