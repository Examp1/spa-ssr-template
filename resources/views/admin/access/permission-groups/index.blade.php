...
@section('content')
    <nav aria-label="breadcrumb" class="breadcrumb-nav">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active" aria-current="page">{{ __('Permissions Groups') }}</li>
        </ol>
    </nav>
            ...
            <div class="card">
                <div class="card-header">
                    @can('permission_groups_create')
                        <a href="{{ route('permission-groups.create') }}" class="btn btn-primary float-right">
                                ...
                                <tr data-id="{{ $item->id }}">
                                    ...
                                    <td>
                                        @can('permission_groups_edit')
                                            <a href="{{ route('permission-groups.edit', $item->id) }}">
                                                {{ $item->name }}
                                            </a>
                                        @else
                                            {{ $item->name }}
                                    ...
                                    <td>
                                        <div style="display: flex">
                                            <div style="margin-left: 10px">
                                                <form action="{{ route('permission-groups.destroy', $item->id) }}"
                                                    method="POST">
                                                    @can('permission_groups_edit')
                                                        <a href="{{ route('permission-groups.edit', $item->id) }}"
