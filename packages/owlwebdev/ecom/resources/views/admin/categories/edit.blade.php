...
@section('content')
    ...
    <form class="form-horizontal" method="POST" action="{{ route('categories.update', $model->id) }}">
        ...
        <div class="row">
                        ...
                        <ul class="nav nav-tabs" id="myTab" role="tablist"
                            ...
                            <li class="nav-item">
                                <a class="nav-link" id="filters-tab" data-toggle="tab" href="#filters" role="tab"
                                    aria-controls="seo" aria-selected="false">Додаткові фільтрації</a>
                            </li>
                        </ul>
                    </div>
                    ...
                    <div class="card-body">
                        <div class="tab-content" id="myTabContent">
                                ...
                                <div class="tab-content">
                                    @foreach ($localizations as $key => $catLang)
                                        <div class="tab-pane p-t-20 p-b-20  @if (config('translatable.locale') == $key) active @endif"
                                            ...
                                            @include('ecom::admin.categories.tabs._main', [
                                                'lang' => $key,
                                                'data' => $data,
                                                'model' => $model,
                                            ])
                                ...
                                <div class="tab-content tabcontent-border">
                                    @foreach ($localizations as $key => $catLang)
                                        <div class="tab-pane p-t-20 p-b-20  @if (config('translatable.locale') == $key) active @endif"
                                            ...
                                            @include('ecom::admin.categories.tabs._seo', [
                                                'lang' => $key,
                                                'data' => $data,
                                            ])
                                        </div>
                            ...
                            <div class="tab-pane fade" id="filters">
                                @include('ecom::admin.categories.tabs._filters', [
                                    'model' => $model,
                                ])
            ...
            @include('ecom::admin.categories.aside')
        </div>
    </form>

    @can('categories_delete')
        <form action="{{ route('categories.destroy', $model->id) }}" method="POST" class="delete-model-form">
            @csrf
            @method('DELETE')
        </form>
    @endcan
