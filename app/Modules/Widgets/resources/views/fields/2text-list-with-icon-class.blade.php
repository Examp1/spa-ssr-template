    ...
    <div class="input-group mb-3">
        <div style="display: none;">
            <div data-item-id="#dynamicListPlaceholder"
                ...
                <div class="col-lg-3 input-group-sm">
                    <input type="text" name="{{ $field['name'] }}[#dynamicListPlaceholder][name]" placeholder="Назва"
                        class="form-control mb-1" disabled>
                </div>

                <div class="col-lg-4 input-group-sm">
                    <input type="text" name="{{ $field['name'] }}[#dynamicListPlaceholder][description]"
                        placeholder="Опис" class="form-control mb-1" disabled>
                </div>
        ...
        <div class="items-container w-100">
            @foreach ((array) old($field['name'], $value) as $key => $value)
                <div data-item-id="{{ $key }}"
                    ...
                    <div class="col-lg-3 input-group-sm">
                        <input type="text" name="{{ $field['name'] }}[{{ $key }}][name]"
                            placeholder="Назва"
                    ...
                    <div class="col-lg-4 input-group-sm">
                        <input type="text" name="{{ $field['name'] }}[{{ $key }}][description]"
                            placeholder="Опис"
                            value="{{ old($field['name'] . '.' . $key . '.description', $value['description'] ?? '') }}"
                            class="form-control mb-1">
                    </div>
