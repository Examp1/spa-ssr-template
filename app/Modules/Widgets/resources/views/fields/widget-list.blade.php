        ...
        <div style="display: none;">
            <div data-item-id="#dynamicListPlaceholder"
                ...
                <div class="col-10 input-group-sm">
                    <select name="{{ $field['name'] }}[#dynamicListPlaceholder][widget]" class="form-control" disabled>
                        @foreach ($field['list']() as $id => $widget)
                            <option value="{{ $id }}">{{ __($widget) }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="col-2">
                    <button type="button"
                        class="btn btn-danger btn-sm remove-item float-right">{{ __('Delete') }}</button>
                </div>
            </div>
        </div>
        ...
        <div class="items-container w-100">
            @foreach ((array) old($field['name'], $value) as $key => $value)
                <div data-item-id="{{ $key }}"
                    class="item-template item-group input-group mb-3 align-items-center border border-grey-light pt-2 pb-2">
                    <div class="col-10 input-group-sm">
                        <select name="{{ $field['name'] }}[{{ $key }}][widget]" class="form-control">
                            @foreach ($field['list']() as $id => $widget)
                                <option value="{{ $id }}" @if ($value['widget'] == $id) selected @endif>
                                    {{ __($widget) }}</option>
                            @endforeach
                        </select>
                    </div>
