<div class="form-group">
        ...
        <div class="items-container w-100">
            @foreach ((array) old($field['name'], $value) as $key => $value)
                <div data-item-id="{{ $key }}"
                    ...
                    <div class="col-10">
                    <select class="select2-field-shown" style="width: 100%" name="{{ $field['name'] }}[{{ $key }}][product_id]">
                        <option value="">---</option>
                        {!! \Owlwebdev\Ecom\Models\Product::getOptionsHTML($value['product_id']) !!}
                    </select>
                    </div>
    ...
    <template id="categories_template">
        <div data-item-id="#dynamicListPlaceholder"
            class="item-template item-group input-group mb-3 align-items-center border border-grey-light pt-2 pb-2">
            <div class="col-10">
                <select class="select2-field" style="width: 100%" name="{{ $field['name'] }}[#dynamicListPlaceholder][product_id]">
                    <option value="">---</option>
                    {!! \Owlwebdev\Ecom\Models\Product::getOptionsHTML() !!}
                </select>
            </div>
            <div class="col-2">
                <button type="button"
