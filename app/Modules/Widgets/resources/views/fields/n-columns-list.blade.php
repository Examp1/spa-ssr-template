<div class="form-group">
    ...
    <div class="input-group mb-3">
        <div style="display: none;">
            @foreach ($field['columns'] as $col_count => $width)
                <div data-item-id="#dynamicListPlaceholder{{ $col_count }}"
                    class="n-column-row-template{{ $col_count }} item-group mb-1 bg-light border border-dark p-1 col-12">
                                    ...
                                    <td>
                                        <label>Колонка<button type="button"
                                                class="btn btn-success btn-sm edit-item ml-1"><i
                                                    class="fas fa-edit text-green"></i></button><button type="button"
                                                class="btn btn-succes btn-sm edit-item-del"><i
        ...
        <div class="n-columns-container w-100">
    ...
    <button type="button"
        class="btn btn-success text-white btn-sm add-n-column-{{ studly_case($field['name']) }}">Додати</button>
</div>

@push('scripts')
    <script type="text/javascript">
        $(document).on('click', '.add-n-column-{{ studly_case($field['name']) }}', function() {
            const count = $(this).closest('.card-body').find('.item-row-template-count>option:selected').val();
            const parent = $(this).parent();
            const template = parent.find('.n-column-row-template' + count);
            const container = parent.find('.n-columns-container');
