        ...
        <div style="display: none;">
            <div data-item-id="#dynamicListPlaceholder" class="item-template item-group input-group mb-3 align-items-center border border-grey-light pt-2 pb-2">
                <div class="col-lg-5 input-group-sm">
                    <input type="text" name="{{ $field['name'] }}[#dynamicListPlaceholder][name]" placeholder="Назва" class="form-control mb-1" disabled>
                </div>

                <div class="col-lg-5 input-group-sm">
                    <input type="text" name="{{ $field['name'] }}[#dynamicListPlaceholder][description]" placeholder="Опис" class="form-control mb-1" disabled>
                </div>
        ...
        <div class="items-container w-100">
            @foreach((array) old($field['name'], $value) as $key => $value)
                <div data-item-id="{{ $key }}" class="item-template item-group input-group mb-3 align-items-center border border-grey-light pt-2 pb-2">
                    <div class="col-lg-5 input-group-sm">
                        <input type="text" name="{{ $field['name'] }}[{{ $key }}][name]" placeholder="Назва" value="{{ old($field['name'] . '.' . $key . '.name', $value['name'] ?? '') }}" class="form-control mb-1">
                    </div>

                    <div class="col-lg-5 input-group-sm">
                        <input type="text" name="{{ $field['name'] }}[{{ $key }}][description]" placeholder="Опис" value="{{ old($field['name'] . '.' . $key . '.description', $value['description'] ?? '') }}" class="form-control mb-1">
                    </div>
