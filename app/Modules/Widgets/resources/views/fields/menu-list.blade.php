    ...
    <div class="input-group mb-3">
        <div style="display: none;">
            <div data-item-id="#dynamicListPlaceholder"
                ...
                <div class="col-lg-5 input-group-sm">
                    <input type="text" name="{{ $field['name'] }}[#dynamicListPlaceholder][link_name]"
                        placeholder="Посилання заголовок" class="form-control mb-1" disabled>
                    <input type="text" name="{{ $field['name'] }}[#dynamicListPlaceholder][link_link]"
                        placeholder="Посилання адрес" class="form-control mb-1" disabled>
                </div>
        ...
        <div class="items-container w-100">
            @foreach ((array) old($field['name'], $value) as $key => $value)
                <div data-item-id="{{ $key }}"
                    ...
                    <div class="col-lg-5 input-group-sm">
                        <input type="text" name="{{ $field['name'] }}[{{ $key }}][link_name]"
                            placeholder="Посилання заголовок"
                            value="{{ old($field['name'] . '.' . $key . '.link_name', $value['link_name'] ?? '') }}"
                            class="form-control mb-1">
                        <input type="text" name="{{ $field['name'] }}[{{ $key }}][link_link]"
                            placeholder="Посилання адрес"
                            value="{{ old($field['name'] . '.' . $key . '.link_link', $value['link_link'] ?? '') }}"
                            class="form-control mb-1">
                    </div>
