...
export const order =  {
  computed: {
    ...
    prefix() {
      return this.getCurrentCurrencies ? this.getCurrentCurrencies.prefix : null
    },
    suffix() {
      return this.getCurrentCurrencies ? this.getCurrentCurrencies.suffix : null
    },
  },
  methods: {
    ...mapMutations({
    ...
    async getCount() {
      ...
      const res = await this.$axios.$post('/api/cart/count', option, this.getBearerToken)
    ...
    async plus(prodId, ev) {
      const prodCount = ++ev.target.parentElement.querySelector('input').value
    ...
    async updateWishlistState(id) {
      if (process.client) {
        if (this.isLogin) {
          const { data } = await this.$axios.post('/api/profile/wishlist/update', {
            product_id: id
          }, this.getBearerToken)
          localStorage.wishList = JSON.stringify(data.data.wishlist_product_ids)
          this.setWishListFromLocalStorage(data.data.wishlist_product_ids);
        } else {
          const w = localStorage.wishList ? JSON.parse(localStorage.wishList) : []
