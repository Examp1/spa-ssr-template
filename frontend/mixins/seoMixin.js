...
export const seoMixin = {
  head() {
    ...
    if (this.$route.query.page && this.$route.query.page > 1) {
      const prevPage = this.products.paginate.prev_page_url?.split('page=')[1]
      const nextPage = this.products.paginate.next_page_url?.split('page=')[1] || this.products.paginate.last_page_url.split('page=')[1]
      links.push(
        { rel: 'prev', href: `${baseUrl}${this.$route.path}?page=${prevPage}` },
        { rel: 'next', href: `${baseUrl}${this.$route.path}?page=${nextPage}` }
      );
    }
